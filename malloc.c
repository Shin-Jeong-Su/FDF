...
/*   Updated: 2024/03/13 20:20:57 by jeshin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	calloc_map_color(t_dq *dq, t_map_info *map)
{
	...
	map->color = (int **)malloc(sizeof(int *) * map->n_row);
	if (map->crd == 0)
	{
		clear_dq(dq);
		return (EXIT_FAILURE);
	...
	while (++i < map->n_row)
	{
		 map->color[i] = (int *)malloc(sizeof(int) * map->n_col);
		 if (map->color[i] == 0)
		 {
			j = -1;
			while(++j < i - 1)
		 ...
		 ft_bzero(map->color[i], map->n_col * sizeof(int));
	}
	return (EXIT_SUCCESS);
}